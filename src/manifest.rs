...
impl Manifest {
    ...
    pub fn write(&self, project_dir: &Path) -> Result<()> {
        ...
        fs::create_dir_all(&claude_dir).with_context(|| {
            format!("creating {}", claude_dir.display())
        })?;
    ...
    pub fn write_template(
        ...
        fs::create_dir_all(templates_dir).with_context(|| {
            format!("creating {}", templates_dir.display())
        })?;
        let path = templates_dir.join(format!("{name}.json"));
        let json = serde_json::to_string_pretty(self)
            .context("serializing template")?;
        fs::write(&path, json).with_context(|| {
            format!("writing template '{name}'")
        })?;
        Ok(())
    }
...
mod tests {
    ...
    fn roundtrip_write_then_read() {
        ...
        let loaded =
            Manifest::read(tmp.path()).unwrap().unwrap();
    ...
    fn template_roundtrip() {
        ...
        let loaded =
            Manifest::read_template(&tpl_dir, "backend")
                .unwrap();
        assert_eq!(loaded.version, 1);
        assert_eq!(
            loaded.commands,
            vec!["trailofbits/config.md"]
        );
        assert_eq!(loaded.skills, vec!["review.md"]);
    }
    ...
    fn read_template_missing_errors() {
        let tmp = tempfile::tempdir().unwrap();
        let result =
            Manifest::read_template(tmp.path(), "nope");
        assert!(result.is_err());
    }
    ...
    fn list_templates_empty_dir() {
        let tmp = tempfile::tempdir().unwrap();
        let names =
            Manifest::list_templates(tmp.path()).unwrap();
    ...
    fn list_templates_nonexistent_dir() {
        ...
        let names =
            Manifest::list_templates(&missing).unwrap();
        assert!(names.is_empty());
    }
    ...
    fn list_templates_returns_sorted_names() {
        ...
        fs::write(
            tpl_dir.join("not-json.txt"),
            "ignored",
        )
        .unwrap();

        let names =
            Manifest::list_templates(&tpl_dir).unwrap();
    ...
    fn delete_template_removes_file() {
        ...
        let manifest = sample_manifest();
        manifest
            .write_template(&tpl_dir, "doomed")
            .unwrap();
        assert!(tpl_dir.join("doomed.json").exists());

        Manifest::delete_template(&tpl_dir, "doomed")
            .unwrap();
        assert!(!tpl_dir.join("doomed.json").exists());
    }
    ...
    fn delete_template_missing_errors() {
        let tmp = tempfile::tempdir().unwrap();
        let result =
            Manifest::delete_template(tmp.path(), "ghost");
        assert!(result.is_err());
    }
}
